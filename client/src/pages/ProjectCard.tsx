    ...
    {gen: Project, setGenerations: React.Dispatch<React.SetStateAction<Project[]>>, forCommunity?:boolean}) => {
        ...
        return (
            <div key={gen.id} className="mb-4 break-inside-avoid">
                <div className="bg-white/5 border border-white/10 rounded-xl overflow-hiden hover:border-white/20 transition group">
                {/*preview*/}
                <div
  className={`${
...
>
                    ...
                    {gen.generatedVideo && (
                        <video src={gen.generatedVideo} muted loop playsInline className="absolute inset-0 w-full object-cover opacity-0 group-hover:opacity-100 transition duration-500"
                    ...
                    <div className="absolute left-3 top-3 flex gap-2 items-center">
                        {gen.isGenerating && (
                            <span className="text xs px-2 py-1 bg-green-600/30 rounded-full">Generating</span>
                        )}
                    ...
                    {!forCommunity &&(
                        ...
                        className="absolute right-3 top-3 sm:opacity-0 group-hover:opacity-100 transition flwx items-center gap-2">
                            <div className="absolute top-3 right-3">
                                <EllipsisIcon className="ml-auto bg-black/10 rounded-full p-1 size-7"/>
                            </div>
                            <div className={`text -xs ${menuOpen ? 'block' : 'hidden'} verflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 blackdrop-blur text-white border border-gray-500/50 rounded-lg shadow-md mt-2 py-1 z-10 text-xs`}>

                            {gen.generatedImage && <a href="#" download className="flex gap-2 items-center px-4 py-2 text-xs hover:bg-black/10 cursor-pointer">
                            <ImageIcon size={14}/>Download Image</a>}

                            {gen.generatedVideo && <a href="#" download className="flex gap-2 items-center px-4 py-2 text-xs hover:bg-black/10 cursor-pointer">
                            <PlaySquareIcon size={14}/>Download video</a>}

                            {(gen.generatedImage || gen.generatedImage)&& 
                            <button onClick={()=> navigator.share({url: gen.generatedImage||gen.generatedVideo , title:gen.productName,text: gen.productDescription})}
                        ...
                        <div className="flex items-start justify-between  gap-4">
                            ...
                            <div className="text-right">
                                <div className="mt-2 flex flex-col items-end gap-1">
                                    <span className="text-xs px-2 py-1 bg-white/5 rounded-full"> Aspect: {gen.aspectRatio}
